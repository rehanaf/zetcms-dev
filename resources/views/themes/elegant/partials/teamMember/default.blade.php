                ...
                @foreach($data['members'] as $member)
                    ...
                    <div class="col-lg-3 col-md-6">
                        <div class="card h-100 border-0 bg-white rounded-4 shadow-sm text-center p-4 feature-card">
                            ...
                            @else
                                <div class="rounded-circle bg-chocolate-dark d-flex align-items-center justify-content-center mx-auto mb-4 border border-2 border-accent" style="width: 120px; height: 120px;">
                                    <span class="text-accent font-serif fs-1">
                            ...
                            @if(!empty($member['name']))
                                <h4 class="font-serif text-chocolate fw-bold m-0">{{ $member['name'] }}</h4>
                            @endif
                            @if(!empty($member['role']))
                                <p class="text-accent font-sans small fw-semibold mb-3">{{ $member['role'] }}</p>
                            @endif
                            @if(!empty($member['bio']))
                            ...
                            <div class="d-flex justify-content-center gap-3 mt-auto">
                                @if(!empty($member['facebook_url']))
                                    <a href="{{ $member['facebook_url'] }}" target="_blank" class="text-muted hover-text-accent transition">
                                        <i class="fa-brands fa-facebook fs-5"></i>
                                    </a>
                                @endif
                                @if(!empty($member['twitter_url']))
                                    <a href="{{ $member['twitter_url'] }}" target="_blank" class="text-muted hover-text-accent transition">
                                        <i class="fa-brands fa-twitter fs-5"></i>
                                    </a>
                                @endif
                                @if(!empty($member['linkedin_url']))
                                    <a href="{{ $member['linkedin_url'] }}" target="_blank" class="text-muted hover-text-accent transition">
                                        <i class="fa-brands fa-linkedin fs-5"></i>
                                    </a>
                                @endif
                                @if(!empty($member['instagram_url']))
                                    <a href="{{ $member['instagram_url'] }}" target="_blank" class="text-muted hover-text-accent transition">
