    ...
    <div class="container py-4 max-w-lg mx-auto">
        ...
        @if(!empty($data['title']) || !empty($data['description']))
            <div class="text-center mb-5">
                @if(!empty($data['title']))
                    <h2 class="display-6 font-serif text-chocolate mb-3">{{ $data['title'] }}</h2>
                @endif
        ...
        @if(!empty($data['rows']))
            <div class="row justify-content-center">
                <div class="col-lg-10">
                    <div class="table-responsive rounded shadow-sm border border-accent border-opacity-25 bg-cream p-3">
                                    ...
                                    <thead style="border-bottom: 2px solid var(--color-accent);">
                                        <tr>
                                            @foreach($cells as $cell)
                                                <th class="py-3 px-4 font-serif text-chocolate fs-5">
                                ...
                                @else
                                    <tr style="border-bottom: 1px solid rgba(212,175,55,0.15);">
                                        @foreach($cells as $cellIndex => $cell)
                                            <td class="py-3 px-4 text-secondary {{ $cellIndex === 0 ? 'fw-semibold text-chocolate' : '' }}">
                                                {{ is_array($cell) ? ($cell['value'] ?? '') : $cell }}
                                            </td>
                                        @endforeach
                                    </tr>
