        ...
        <div class="row justify-content-center">
            <div class="col-lg-9">
                ...
                <div class="text-center mb-5">
                    ...
                    <h1 class="display-4 font-serif text-chocolate fw-bold mb-4">{{ $post->title }}</h1>
                    
                    <div class="d-flex align-items-center justify-content-center gap-3 font-sans text-muted small">
                        <div class="d-flex align-items-center gap-2">
                            <div class="rounded-circle bg-chocolate-dark text-accent d-flex justify-content-center align-items-center fw-bold" style="width: 32px; height: 32px;">
                                {{ substr($post->user->name ?? 'A', 0, 1) }}
                            </div>
                            <span class="fw-semibold text-chocolate">{{ $post->user->name ?? 'Administrator' }}</span>
                        </div>
                        <span>•</span>
                ...
                @if($post->relatedPosts->isNotEmpty())
                    <div class="mt-5 pt-5 border-top border-accent border-opacity-25">
                        <h3 class="font-serif text-chocolate mb-4">Baca Juga</h3>
                        <div class="row g-4">
                            @foreach($post->relatedPosts as $related)
                                <div class="col-md-6">
                                    <div class="d-flex gap-3 align-items-center bg-white p-3 rounded shadow-sm hover-shadow transition h-100">
                                        ...
                                        @else
                                            <div class="rounded bg-cream d-flex align-items-center justify-content-center text-muted fw-bold font-serif" style="width: 80px; height: 80px;">ZC</div>
                                        @endif
                                        <div>
                ...
                {{-- Comments --}}
                <div class="mt-5 pt-5 border-top border-accent border-opacity-25">
                    <h3 class="font-serif text-chocolate mb-4">Diskusi & Komentar</h3>
                                ...
                                <div class="bg-white p-4 rounded shadow-sm mb-4">
                                    <div class="d-flex justify-content-between align-items-start mb-3">
                                        <div class="d-flex align-items-center gap-3">
                                            <div class="rounded-circle bg-cream text-chocolate d-flex align-items-center justify-content-center fw-bold font-serif fs-5" style="width: 45px; height: 45px;">
                                                {{ substr($comment->author_name, 0, 1) }}
                                            </div>
                                            <div>
                                                <h6 class="mb-0 font-serif fw-bold text-chocolate">{{ $comment->author_name }}</h6>
                                                <small class="text-muted font-sans">{{ $comment->created_at->diffForHumans() }}</small>
                                            </div>
                                        </div>
                                        <button onclick="setParentId({{ $comment->id }}, '{{ $comment->author_name }}')" class="btn btn-sm btn-link text-accent text-decoration-none fw-bold font-sans">Balas</button>
                                    </div>
                                            ...
                                            @foreach($comment->replies->where('status', 'approved') as $reply)
                                                <div class="mb-3">
                                                    <div class="d-flex align-items-center gap-2 mb-2">
                                                        <div class="rounded-circle bg-chocolate text-white d-flex align-items-center justify-content-center fw-bold" style="width: 30px; height: 30px; font-size: 0.8rem;">
                                                            {{ substr($reply->author_name, 0, 1) }}
                                                        </div>
                                                        <h6 class="mb-0 font-serif fw-bold text-chocolate fs-6">{{ $reply->author_name }}</h6>
                    ...
                    {{-- Form Komentar --}}
                    <div class="mt-4 pt-4">
                        <h4 class="font-serif text-chocolate fw-bold mb-4" id="form-title">Tinggalkan Pesan</h4>
                        
                        <div id="reply-banner" class="d-none alert alert-light bg-cream text-chocolate py-2 px-3 border-accent d-flex justify-content-between align-items-center font-sans mb-4">
                        ...
                        <form action="{{ route('blog.comment', $post->slug) }}" method="POST">
                            ...
                            <button type="submit" class="btn btn-elegant px-5 py-2 font-sans fw-bold">Kirim Pesan</button>
                        </form>
                    </div>
                </div>
