                        ...
                        @foreach($data['faqs'] as $i => $faq)
                            <div class="accordion-item border-0 mb-3 rounded shadow-sm bg-main">
                                <h2 class="accordion-header" id="heading{{ $i }}">
                                    <button class="accordion-button collapsed bg-transparent fw-bolder tracking-tight text-primary fw-bold fs-5 shadow-none" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $i }}" aria-expanded="false" aria-controls="collapse{{ $i }}">
                                        {{ $faq['question'] }}
                                    </button>
                                </h2>
                                <div id="collapse{{ $i }}" class="accordion-collapse collapse" aria-labelledby="heading{{ $i }}" data-bs-parent="#faqAccordion">
                                    <div class="accordion-body  text-secondary border-top border-primary border-opacity-25 pt-4">
                                        {{ $faq['answer'] }}
                                    </div>
                                </div>
                            </div>
