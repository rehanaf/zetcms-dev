                        ...
                        <div class="card h-100 border-0 rounded-4 {{ $plan->is_featured ? 'shadow-lg bg-surface-elevated text-light' : 'shadow-sm bg-main' }}" style="transition: transform 0.3s ease; {{ $plan->is_featured ? 'transform: scale(1.05); z-index: 2;' : '' }}">
                            <div class="card-body p-5 d-flex flex-column position-relative">
                                    ...
                                    <ul class="list-unstyled mb-5 flex-grow-1">
                                        @foreach((array) $plan->features as $feat)
                                            <li class="mb-3 d-flex align-items-center {{ $plan->is_featured ? 'text-light' : 'text-secondary' }}">
                                                <i class="fa-solid fa-check text-primary text-gradient me-3"></i> {{ is_array($feat) ? ($feat['name'] ?? json_encode($feat)) : $feat }}
                                            </li>
                                        @endforeach
                                    </ul>
                                @endif
                                ...
                                @if($plan->button_text)
                                    <div class="mt-auto">
                                        @if($plan->is_featured)
                                            <a href="{{ $plan->button_url ?? '#' }}" class="btn-gradient w-100 text-center d-block">
                                                {{ $plan->button_text }}
                                            </a>
                                        @else
