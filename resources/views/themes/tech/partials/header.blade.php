<nav class="navbar navbar-expand-lg navbar-dark fixed-top">
    <div class="container">
        <a class="navbar-brand d-flex align-items-center" href="/">
            ...
            @else
                <span class="text-primary text-gradient fw-bold m-0 pe-2">{{ $siteName }}</span>
            @endif
        </a>
        ...
        <div class="collapse navbar-collapse" id="navbarContent">
            <ul class="navbar-nav ms-auto mb-2 mb-lg-0 align-items-center">
                        ...
                        @if($item->children->count() > 0)
                            <li class="nav-item dropdown">
                                <a class="nav-link dropdown-toggle" href="{{ $item->url }}" id="navbarDropdown{{ $loop->index }}" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                    {{ $item->title }} <i class="fa-solid fa-chevron-down ms-1 fs-8 text-primary text-gradient"></i>
                                </a>
                ...
                <li class="nav-item ms-lg-3 mt-3 mt-lg-0">
                    <a class="btn-gradient py-2 px-4" href="{{ $headerBtnUrl }}">{{ $headerBtnText }}</a>
                </li>
                @endif
            </ul>
        </div>
...
<div class="offcanvas-drawer" id="mobileDrawer">
    ...
    <div class="drawer-nav">
                ...
                @if($item->children->count() > 0)
                    <div>
                        <a class="drawer-link" id="drawerDropdownToggle{{ $loop->index }}">
                            {{ $item->title }} <i class="fa-solid fa-chevron-down fs-8 text-primary text-gradient transition-transform" id="drawerChevron{{ $loop->index }}" style="transition: transform 0.3s ease;"></i>
                        </a>
        ...
        <div class="mt-4 pt-3 border-top border-secondary border-opacity-25">
            <a class="btn-gradient py-3 px-4 w-100 text-center d-block" href="{{ $headerBtnUrl }}">{{ $headerBtnText }}</a>
        </div>
        @endif
    </div>
</div>
