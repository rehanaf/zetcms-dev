        ...
        <div class="row align-items-center g-5">
            <div class="col-lg-6" data-aos="fade-right">
                @if(!empty($data['subtitle']))
                    <p class="text-primary text-gradient fw-bold mb-2 fs-6" style="letter-spacing: 2px;">{{ $data['subtitle'] }}</p>
                @endif
                
                @if(!empty($data['title']))
                    <h2 class="display-5 text-primary mb-3">{{ $data['title'] }}</h2>
                @endif
                ...
                @if(!empty($data['button_text']))
                    <a href="{{ $data['button_url'] ?? '#' }}" class="btn-gradient">
                        {{ $data['button_text'] }}
                    </a>
                @endif
            </div>
